 ...
 *
 * The app redirects unauthenticated users to Google's authorization endpoint
 * with PKCE challenge parameters. After the user authenticates and consents,
 * Google redirects back with an authorization code. The app exchanges the code
...
export const STEPS: Step[] = [
	{
		...
		securityNote:
			'PKCE (Proof Key for Code Exchange, RFC 7636) protects against authorization code interception attacks. The app generates a random code_verifier (43-128 chars), computes code_challenge = BASE64URL(SHA256(code_verifier)), and sends only the challenge to Google. The verifier is stored server-side and sent during token exchange. The state parameter prevents CSRF; the nonce prevents ID token replay. Note what is absent: access_type=offline. A refresh token is only needed to call Google APIs while the user is away. Requesting one for a login-only flow buys nothing and adds storage, rotation, revocation, and breach-handling obligations, so treat offline access as an opt-in taken when a specific background call requires it.',
		http: [
			{
				type: 'request',
				from: 'Browser',
	...
	},
	{
		...
		description:
			'Browser follows the redirect to Google. If the user is already signed into Google, they see the OAuth consent screen showing which permissions the app is requesting (email, profile). Otherwise, they see the Google sign-in page first.',
		securityNote:
		...
		http: [
			...
			},
			{
				...
				to: 'Browser',
				status: '200 OK',
				headers: ['Content-Type: text/html', 'Set-Cookie: __Host-GAPS=...; HttpOnly; Secure; SameSite=None'],
	...
	},
	{
		...
		urlBar: 'https://accounts.google.com/o/oauth2/v2/auth?client_id=...',
		description:
			"User signs into their Google account (if not already signed in) and approves the requested permissions. Google issues a single-use authorization code and redirects the browser back to the app's callback URL with the code and the original state parameter.",
		securityNote:
			"The authorization code is short-lived (typically 10 minutes) and single-use. Google includes the state parameter unchanged in the redirect, allowing the app to verify it matches the value from step 1. If state doesn't match, the app must reject the request as a potential CSRF attack.",
		http: [
			{
				type: 'request',
				from: 'Browser',
			...
			},
			{
				...
				to: 'Browser',
				status: '302 Found',
				headers: ['Location: https://secrets.example.com/auth/callback?code=4/0AeanS0a...&state=xYz9Kp2mN7qR4sT1'],
	...
	},
	{
		...
		description:
			"Browser follows the redirect back to the app. The app validates the state parameter, then exchanges the authorization code for tokens via a server-to-server POST to Google's token endpoint. The code_verifier (PKCE proof) is included to prove the app is the same party that initiated the flow.",
		securityNote:
			'The token exchange is server-to-server (back-channel), so the browser never sees the tokens. The app sends both the client_secret (confidential client authentication) and the code_verifier (PKCE proof). Google verifies BASE64URL(SHA256(code_verifier)) matches the code_challenge from step 1. This dual verification prevents both code interception and client impersonation.',
		http: [
			{
				type: 'request',
				from: 'Browser',
...
// Verify with JWKS: https://www.googleapis.com/oauth2/v3/certs`,
	...
	{
		id: 5,
		title: 'The app validates ID token and fetches userinfo',
		userSees: 'loading',
		urlBar: 'https://secrets.example.com/auth/callback?code=4/0AeanS0a...&state=xYz9Kp2mN7qR4sT1',
		description:
			"The app validates the ID token JWT (signature, issuer, audience, nonce, expiry) using Google's public keys. It then optionally calls the userinfo endpoint with the access_token to fetch additional profile data that may not be in the ID token.",
		securityNote:
		...
		http: [
			...
			},
			{
				...
				headers: ['Authorization: Bearer ya29.a0AfB_byC...'],
  ...
  "locale": "en"
	...
	},
	{
		...
		urlBar: 'https://secrets.example.com/dashboard',
		description:
			'The user is authenticated and reaches their dashboard. The app reads the session to render content personalized with the Google profile data (name, email, avatar).',
		securityNote:
			'The access_token stays server-side, never exposed to the browser. Since login is finished, the app can drop it entirely; keeping it only makes sense if there is a Google API to call. Revoke any token still held on logout via https://oauth2.googleapis.com/revoke.',
		http: [
			{
				type: 'request',
				from: 'Browser',
	...
	},
	{
		...
		urlBar: 'https://secrets.example.com/dashboard',
		description:
			'All future requests use the session cookie. No further interaction with Google until the session expires or the user logs out. The app session is entirely local: Google has no way to end it, and Google token expiry does not end it either.',
		securityNote:
