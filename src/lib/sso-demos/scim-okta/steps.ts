 ...
 * The app as the SCIM server (base URL: https://secrets.example.com/scim/v2).
 *
 ...
 * call from Okta to the app, authenticated with a static Bearer token.
 *
...
export const STEPS: Step[] = [
	{
		id: 1,
		title: 'Admin assigns Alice to the app in Okta',
		userSees: 'okta-admin',
		...
		http: [
			{
				...
				note: 'Admin console assigns Okta user 00u1abcd2EFGHIJKL345 (alice@contoso.com) to the app',
			},
			{
				type: 'internal',
				from: 'Okta',
	...
	},
	{
		...
		urlBar: 'https://contoso-admin.okta.com/admin/app/ots_scim/instance/0oa7xyz/#tab-assignments',
		description:
			'Before creating anything, Okta asks the app whether an account with this userName already exists: GET /Users with a filter expression. This is the correlation mechanism of the whole integration. SCIM has no shared database -- the only way Okta and the app agree they are talking about the same person is by matching userName. If the app already had an alice@contoso.com (created manually, or by a previous integration), Okta would link to that existing account instead of creating a duplicate. Here the ListResponse comes back with totalResults 0: Alice is unknown to the app, so Okta will create her.',
		securityNote:
	...
	},
	{
		...
		urlBar: 'https://contoso-admin.okta.com/admin/app/ots_scim/instance/0oa7xyz/#tab-assignments',
		description:
			'No existing account matched, so Okta POSTs a full User resource to the app. The payload uses the core User schema from RFC 7643: userName as the login identifier, name components, a work email, active true, and externalId set to Alice’s Okta user ID. The app validates the payload, creates the account, and responds 201 Created with the stored resource -- now carrying a server-assigned id and a meta block with the resource’s canonical location. The identifier ownership is deliberate and worth internalizing: the client owns externalId (Okta’s stable ID for Alice, immune to email changes), the server owns id (the app’s primary key). Every subsequent call from Okta targets /Users/{id} using the server’s identifier, not its own.',
		securityNote:
  ...
  "meta": {
    ...
    "location": "https://secrets.example.com/scim/v2/Users/8c1f9a2e-4b7d-4e3a-9c0d-2f5e8a716b43"
				...
				note: 'Okta stores the returned id and uses it for all future calls about Alice. externalId (Okta’s ID) and id (the app’s ID) now cross-reference each other.',
			},
		],
		actors: {
			browser: false,
  ...
  "active": true
	...
	{
		id: 4,
		title: 'Alice appears in the app -- provisioned, never logged in',
		userSees: 'ots-team',
		urlBar: 'https://secrets.example.com/account/team',
		description:
			'The app’s team-members page now lists Alice: account created, role assigned, last login "never". This is the core mental model of the demo -- provisioning is not authentication. SCIM built the account; nothing has signed into it. When Alice eventually clicks the tile in her Okta dashboard, the SAML flow from the companion demos runs, and the assertion’s NameID (alice@contoso.com) matches the userName SCIM wrote here. The two protocols never talk to each other directly; they rendezvous on that string.',
		securityNote:
		...
		http: [
			{
				...
				note: 'An app admin (separate person, separate session) views the team page',
			},
			{
				type: 'response',
				from: 'App',
	...
	},
	{
		...
		securityNote:
			'PUT-as-full-replace means the provisioning client’s attribute mappings effectively own the user record. If the app let users self-edit their display name, the next Okta push overwrites it with whatever Okta has. Design rule for SCIM servers: partition attributes into IdP-owned (replaced on PUT) and app-owned (stored outside the SCIM-mapped set), and never blend the two in one column. The meta.version weak ETag in responses exists so clients that care can do If-Match concurrency control -- Okta does not send If-Match, which is another way of saying last-write-wins, and Okta writes last.',
		http: [
			{
				type: 'request',
				from: 'Admin Browser',
  ...
  "active": true
				...
				note: 'Only familyName and displayName changed, but the entire resource is sent. Anything the app stored in a SCIM-mapped attribute that is absent here is unasserted, and the server is permitted (not required) to clear it or reset it to a default.',
			},
			{
				type: 'server-response',
				from: 'App',
  ...
  "active": true
	...
	{
		id: 6,
		title: 'Group push: map an Okta group to an app role',
		userSees: 'okta-admin',
		...
		http: [
			{
				...
				note: 'Admin enables Group Push for the "Onetime Secret Admins" Okta group',
			},
			{
				type: 'server',
				from: 'Okta',
				...
				body: '{\n  "schemas": ["urn:ietf:params:scim:schemas:core:2.0:Group"],\n  "displayName": "Onetime Secret Admins",\n  "members": []\n}',
  ...
  "Operations": [
      ...
      "value": [
        {
          ...
          "display": "alice@contoso.com"
				...
				note: 'Members are referenced by app-side user id -- the id from step 3, not the Okta id',
			},
			{
				type: 'server-response',
				from: 'App',
				...
				status: '200 OK',
				headers: ['Content-Type: application/scim+json'],
				note: 'The app maps membership in "Onetime Secret Admins" to its admin role: Alice is now an app admin without anyone touching the app',
			},
		],
		actors: {
			browser: true,
	...
	},
	{
		...
		urlBar: 'https://contoso-admin.okta.com/admin/app/ots_scim/instance/0oa7xyz/#tab-assignments',
		description:
			'Alice leaves the company. The admin (or an HR-driven workflow) deactivates her in Okta, which unassigns her from the app. Okta does not send DELETE /Users/{id} -- it sends PATCH with a single replace operation setting active to false. This soft-delete convention is near-universal among IdPs, and for good reason: hard deletion destroys audit history, orphans owned resources (Alice’s secrets, links, audit trail), and is unrecoverable when the offboarding turns out to be a mistake or a rehire. The app keeps the record, blocks authentication against it, and can reactivate it later with the same PATCH in reverse. A SCIM server should still implement DELETE for spec compliance, but active: false is the path that actually runs in production.',
		securityNote:
			'The classic gap: deactivation is NOT session revocation. This PATCH tells the app "do not let Alice in again" -- it says nothing about the app session cookie already living in her browser from this morning’s login. Unless the app terminates existing sessions when active flips to false, a deprovisioned user retains access until her session expires, which for long-lived sessions can be days. Well-built SCIM servers treat active: false as a two-part command: flag the account AND revoke its live sessions and API tokens. Verify this behavior explicitly when evaluating any SCIM implementation; the spec does not require it and many implementations skip it.',
		http: [
			{
				type: 'request',
				from: 'Admin Browser',
				...
				note: 'Admin unassigns Alice from the app in the Okta console',
			},
			{
				type: 'server',
				from: 'Okta',
      ...
      "value": {
        "active": false
			...
			},
			{
				...
				note: 'Set active=false, block future logins, terminate live sessions and API tokens for this user. The session revocation is the app going beyond what SCIM mandates -- and it is the part that matters most.',
			},
			{
				type: 'server-response',
				from: 'App',
	...
	},
	{
		...
		http: [
			...
			},
			{
				...
				status: '200 OK',
				headers: ['Content-Type: text/html'],
				note: 'Team list shows alice@contoso.com: Deactivated (via SCIM). Account retained for audit; login blocked at both Okta and the app.',
			},
		],
		actors: {
			browser: true,
