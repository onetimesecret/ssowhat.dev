...
export const STEPS: Step[] = [
  ...
  <samlp:SessionIndex>_session_okta_ghi789</samlp:SessionIndex>
	...
	},
	{
		...
		urlBar: 'https://contoso.okta.com/app/ots-saml/exk1234/slo/saml',
		description:
			'Okta renders a page with a hidden iframe per session participant, each loading that SP’s SLO endpoint with a signed LogoutRequest. This is front-channel logout: the user’s own browser is the messenger. The Wiki’s iframe loads, and the LogoutRequest inside it names Alice by NameID and SessionIndex, which is enough to identify the session server-side. The Wiki’s own cookie is SameSite=Lax, so the browser neither sends it in the third-party iframe nor lets the iframe response expire it: Chrome rejects Set-Cookie for SameSite=Lax cookies in cross-site responses, and Safari and Firefox block third-party cookie writes outright. The browser cookie survives no matter what the Wiki does here, which is why revocation has to happen server-side. The Wiki fails because it never stored a mapping from NameID and SessionIndex to its local sessions, so it has nothing to look the session up by. It returns 200 and revokes nothing.',
		securityNote:
