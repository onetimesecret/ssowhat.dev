 ...
 *
 * The app serves many customer organizations, each with its own IdP. Instead of
 * a wall of "Sign in with X" buttons, the app asks for the user's work email
...
export const STEPS: Step[] = [
	...
	},
	{
		...
		http: [
			...
			},
			{
				...
				note: "Redirect to Contoso's Entra tenant. The app also records realm=contoso.com in the pending auth state for later verification.",
			},
		],
		actors: {
			browser: true,
	...
	},
	{
		...
		description:
			"Alice lands on her organization's Microsoft sign-in page with her email prefilled from login_hint. She authenticates under Contoso's Conditional Access policies (MFA, device checks), and Entra redirects back to the app with a single-use authorization code. See the Entra OIDC demo for this leg in full detail.",
		securityNote:
			"From here to the callback, this is plain OIDC -- discovery changed nothing about the protocol. What the app delegated is authentication policy: how hard it is to sign in as alice@contoso.com is now Contoso's decision, enforced in Contoso's tenant, invisible to the app.",
		http: [
			{
				type: 'request',
				from: 'Browser',
			...
			},
			{
				...
				note: 'Single-use authorization code, redirect back to the app',
			},
		],
		actors: {
			browser: true,
  ...
  "ver": "2.0"
	...
	},
	{
		...
		description:
			'Alice is signed in. On her next visit, the _ots_realm hint cookie lets the app offer "Continue with Contoso SSO" immediately instead of asking for her email again -- discovery is a one-time cost per browser.',
		securityNote:
	...
	},
	{
		...
		urlBar: 'https://secrets.example.com/signin',
		description:
			"A different user in a different browser: Bob types bob@acme.com into the same form. This time the registry maps acme.com to Acme's Okta org -- a different vendor and a different protocol. The app generates a SAML AuthnRequest instead of an OIDC redirect and sends Bob to acme.okta.com.",
		securityNote:
	...
	},
	{
		...
		description:
			"Bob signs in under Acme's Okta policies. This reconstructed Classic Engine trace first returns a one-time sessionToken, redeems it for an Okta browser session, and then returns an auto-submitting form that POSTs a signed SAML assertion to the app's ACS endpoint. Identity Engine hosted login uses a different IDX sequence. The app runs the full SAML validation stack (see the SP-initiated SAML demo), plus the same realm-binding check Alice's flow needed: the assertion must come from the entityID registered for acme.com. The NameID here is an opaque persistent identifier, so the email address travels as an ordinary attribute and never acts as the key.",
		securityNote:
